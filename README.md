...
* **Validação e Teste:** Divisão dos dados em 70% para treino, 15% para validação e 15% para testes (ou K-Fold). Os hiperparâmetros são otimizados via `GridSearchCV` dentro de um pipeline anti-leakage da biblioteca `imblearn`.o de um pipeline anti-leakage da biblioteca `imblearn`.
