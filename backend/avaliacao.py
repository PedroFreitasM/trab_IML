...
def avaliar_cascata(df, idx_teste, bundle1, bundle2, caminho_imagem, titulo):
    """Avalia a cascata Etapa 1 + Etapa 2 fim-a-fim no holdout canônico.

    Aplica a Etapa 1 (detecção binária) e, para os fluxos previstos como
    ataque, a Etapa 2 (identificação do tipo), medindo a performance combinada
    exatamente sobre as linhas do holdout (jamais vistas no treino). Gera a
    matriz de confusão da cascata em caminho_imagem.
    """
    ...
    # Executar Predição da Etapa 1 (Detecção)
    ...
    # Executar Predição da Etapa 2 (Identificação)
    ...
    # Relatório de Ataques Perdidos (Falsos Negativos da Etapa 1)
